...
public:
     ...
     */
    template<class... AN>
    static auto everyframe(long count, AN**...)
        -> decltype(rxs::everyframe(count)) {
        return      rxs::everyframe(count);
        static_assert(sizeof...(AN) == 0, "everyframe(period) was passed too many arguments.");
    }
    /*! @copydoc rx-everyframe.hpp
     */
    template<class Coordination>
    static auto everyframe(long count, Coordination cn)
        -> decltype(rxs::everyframe(count, std::move(cn))) {
        return      rxs::everyframe(count, std::move(cn));
    }
    /*! @copydoc rx-everyframe.hpp
     */
    template<class... AN>
    static auto everyframe(rxsc::scheduler::clock_type::time_point initial, long count, AN**...)
        -> decltype(rxs::everyframe(initial, count)) {
        return      rxs::everyframe(initial, count);
        static_assert(sizeof...(AN) == 0, "everyframe(initial, period) was passed too many arguments.");
    }
    /*! @copydoc rx-everyframe.hpp
     */
    template<class Coordination>
    static auto everyframe(rxsc::scheduler::clock_type::time_point initial, long count, Coordination cn)
        -> decltype(rxs::everyframe(initial, count, std::move(cn))) {
        return      rxs::everyframe(initial, count,std::move(cn));
    }
